  ...
  .superRefine((config, context) => {
    if (config.FILE_STORAGE_DRIVER === 's3' && !config.AWS_S3_BUCKET?.trim()) {
      context.addIssue({
        ...
        path: ['AWS_S3_BUCKET'],
        message: 'AWS_S3_BUCKET is required when FILE_STORAGE_DRIVER=s3',
      });
    }

    const hasAccessKey = Boolean(config.AWS_ACCESS_KEY_ID?.trim());
    const hasSecretKey = Boolean(config.AWS_SECRET_ACCESS_KEY?.trim());
    if (hasAccessKey !== hasSecretKey) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['AWS_ACCESS_KEY_ID'],
        message: 'AWS access key and secret key must be configured together',
      });
    }

    if (
      config.NODE_ENV === 'production' &&
      config.FILE_STORAGE_DRIVER === 's3' &&
      ...
      context.addIssue({
        ...
        path: ['AWS_ACCESS_KEY_ID'],
        message:
          'Static AWS credentials are not allowed in production; attach an IAM role to the service',
      });
    }
  });
